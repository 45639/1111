...
updateMatricesAndMovingObjs( VecPosition& fieldXPlusYPlus,
    ...
    for( int i = WO_BALL; i <= WO_OPPONENT_FOOT_R6; ++i ) {
        ...
        if( pObj->currentlySeen ) {
            ...
            } else if( WO_OPPONENT1 <= pObj->id && pObj->id <= WO_OPPONENT6 ) {

                setOpponent( pObj->id, objGlobal );

            } else if( WO_TEAMMATE1 <= pObj->id && pObj->id <= WO_TEAMMATE6 ) {

                setTeammate( pObj->id, objGlobal );

            } else if( WO_TEAMMATE_HEAD1 <= pObj->id && pObj->id <= WO_OPPONENT_FOOT_R6 ) {
